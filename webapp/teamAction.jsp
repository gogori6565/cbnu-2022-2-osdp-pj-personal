...
<body>
	<%
		...
		if(userID == null){ //로그인 X
	    	...
	    	String teamuserID = request.getParameter("userID");
	    	int teamCheck=0;
			
			TeamDAO teamDAO = new TeamDAO();
			int result=teamDAO.setTeam(bbsID, teamuserID, teamCheck);
			
			if(result==-1){ //데이터베이스 오류
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('팀신청에 실패했습니다.')");
				...
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('팀신청에 성공했습니다.')");
